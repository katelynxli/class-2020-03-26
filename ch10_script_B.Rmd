...
x <- train %>%
  group_by(treatment) %>%
  summarize(mean_att_start = mean(att_start), mean_att_end = mean(att_end), mean_att_chg = mean(att_chg)) %>%
  select(treatment,  mean_att_chg) %>%
  pivot_wider(names_from = treatment,
              values_from = mean_att_chg)
...
```
We'd assume control mean_att_chg = 0, but it's -0.40625. Could be confounding factors?
...
```
Bayesian definition: We're 99% confident that the true average difference in attitude changes between the treated and control groups is between 0.136 and 1.485.

Frequentist definition:
99% of the time under repeated redraws of this data, they will contain the true, fixed value of the mean.
...
Units: Each individual that participates in the study.
Treatments: Exposure to a pair of Spanish speaking individuals.
Outcomes: Change in attitude on immigration.
*causal effect*: Whether the exposure to the Spanish speaking individuals results in a change in attitude on immigration.
*fundamental problem of causal inference*: You can't directly observe causal effects at a unit-level, because you cannot give a single subject both the treatment and the control.
...
ggplot(data = train, aes(x = treatment, y = att_chg)) +
  geom_point() +
  geom_jitter(width = 0.02) +
  geom_point(aes(x = "Control", y = x$Control), color = "blue") +
  geom_point(aes(x = "Treated", y = x$Treated), color = "pink")

```
If people in the Treatment group were put in the Control group, their attitude would not have changed (att_chg = 0). 
